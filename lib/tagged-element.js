...
export default class TaggedElement {
   ...
   */
  get range() {
    if (this.consistsOfOneTag) {
      return this.tags[0].range;
    } else {
      return new Range(this.tags[0].range.start, this.tags[1].range.end);
    }
  }
  ...
  get contentRange() {
    if (this.consistsOfOneTag) {
      ...
      return new Range(this.tags[0].range.end, this.tags[1].range.start);
    }
  }
